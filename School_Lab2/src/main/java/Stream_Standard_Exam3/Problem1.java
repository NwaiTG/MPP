...
public class Problem1 {
	...
	public static List<String> elementsInBoth(List<String> list1, List<String> list2) {
		//implement
		return Stream.concat(list1.stream().filter(l1Val -> list2.contains(l1Val)), list2.stream().filter(l2Val -> list1.contains(l2Val)))
				.distinct()
				.toList();
	}
	...
	public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
		//implement
		return list.stream()
				.filter(cus -> cus.getCity().length() >= 6 && !cus.getCity().contains("e"))
				.map(Customer::getZip)
				.distinct()
				.sorted()
				.toList();
